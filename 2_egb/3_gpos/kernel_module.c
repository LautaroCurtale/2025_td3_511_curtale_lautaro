...
static int pos_uart_probe(struct serdev_device *serdev) {
...
static void pos_uart_remove(struct serdev_device *serdev) {
    printk(KERN_INFO "%s: UART cerrada (POS_CTRL)\n", AUTHOR);
    serdev_device_close(serdev);
}

static size_t pos_uart_recv(struct serdev_device *serdev, const unsigned char *buffer, size_t size) {
