...
static uint8_t as5600_read_reg(uint8_t reg) {
    uint8_t val;
    i2c_write_blocking(i2c_port, AS5600_ADDRESS, &reg, 1, true);
    i2c_read_blocking(i2c_port, AS5600_ADDRESS, &val, 1, false);
    return val;
}

// Devuelve el ángulo absoluto en grados (0° a 360°)
